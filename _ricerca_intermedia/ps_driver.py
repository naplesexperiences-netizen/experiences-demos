#!/usr/bin/env python3
"""Esegue PageSpeed (5 thread) su salerno_amalfi_cilento_RAW.tsv.
Uso: PAGESPEED_API_KEY=xxx python3 ps_driver.py
Input  : salerno_amalfi_cilento_RAW.tsv  (col: area, citta, nome, url, categoria, email, note)
Output : salerno_amalfi_cilento_pagespeed.tsv (col: area,citta,name,url,perf,seo,bp,stack,booking,images)
"""
...
IN=os.path.join(HERE,'salerno_amalfi_cilento_RAW.tsv')
OUT=os.path.join(HERE,'salerno_amalfi_cilento_pagespeed.tsv')
